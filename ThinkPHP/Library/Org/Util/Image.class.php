...
class Image {
     ...
     * @return void
     */
    static function thumb3($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 50, $interlace = true) {
...
//            if ($type == "gif") {
//              
//                $thumbImg = new Imagick($image);
//                //$thumbImg = $image->coalesceImages();
//                foreach ($thumbImg as $frame) {
//                    $frame->thumbnailImage($maxWidth, $maxHeight);
//                }
//
//                //$image = $image->optimizeImageLayers();
//                $thumbImg->writeImages($thumbname, true);
//                return $thumbname;
//            } else {
                ...
                // 复制图片
                if (function_exists("ImageCopyResampled"))
                    imagecopyresampled($thumbImg, $srcImg, 0, 0, 0, 0, $maxWidth, $maxHeight, $srcWidth, $srcHeight);
                else
                    imagecopyresized($thumbImg, $srcImg, 0, 0, 0, 0, $maxWidth, $maxHeight, $srcWidth, $srcHeight);
                if ('gif' == $type || 'png' == $type) {
                    //imagealphablending($thumbImg, false);//取消默认的混色模式
                    //imagesavealpha($thumbImg,true);//设定保存完整的 alpha 通道信息
                    $background_color = imagecolorallocate($thumbImg, 0, 255, 0);  //  指派一个绿色
