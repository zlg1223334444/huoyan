...
class Authcode{
	...
	function __construct($length=6,$width=120,$height=40,$isck=true,$key='code'){				
        $this->size=28;
		$this->length=$length;		
		$this->width =$width;
		$this->height=$height;
	...
	function init(){
		...
		if(!empty($this->isBgImg))$this->setBgImg();		
	...
	function draw($info){
		$im=$this->image;       		
		if(!empty($this->isBgNoisy))$this->setBgNoisy();					
		foreach ($info as $v){
			ImageTTFText($im,$v['size'],$v['angle'],$v['x'],$v['y'],$v['color'],$v['font'],$v['text']);
		}
	    $this->setImg();
        if(!empty($this->isPixel))$this->setPixel();
		if(!empty($this->isLine1))$this->setLine1();
		if(!empty($this->isLine2))$this->setLine2();								
		$this->display();
	}
	...
	function setBgImg(){
		//图片文件必须是绝对路径
		$bgname=array('Public/authcode/bgpic1.jpg','Public/authcode/bgpic2.jpg','Public/authcode/bgpic3.jpg');
	...
	function getFontArray(){
		//http://www.font5.com/font.php?tag=%B8%AF%CA%B4%D7%D6%CC%E5&page=2	
		//字体文必须是绝对路径        
	    $f[]=array('font'=>'Public/authcode/1.ttf','size'=>28,'img'=>1,'pixel'=>0,'line1'=>0,'line2'=>1,'bgnoisy'=>0,'pd'=>0.8); //**  t 28		
		$f[]=array('font'=>'Public/authcode/2.ttf','size'=>28,'img'=>0,'pixel'=>0,'line1'=>0,'line2'=>0,'bgnoisy'=>0,'pd'=>0.8); //***** 28
		$f[]=array('font'=>'Public/authcode/3.ttf','size'=>16,'img'=>0,'pixel'=>1,'line1'=>0,'line2'=>1,'bgnoisy'=>0,'pd'=>0.8); //**** 16
		$f[]=array('font'=>'Public/authcode/4.ttf','size'=>58,'img'=>0,'pixel'=>0,'line1'=>0,'line2'=>0,'bgnoisy'=>0,'pd'=>0.8); //*** 58			
		$f[]=array('font'=>'Public/authcode/5.ttf','size'=>26,'img'=>1,'pixel'=>0,'line1'=>0,'line2'=>0,'bgnoisy'=>0,'pd'=>0.9); //*** t 26 
		$f[]=array('font'=>'Public/authcode/6.ttf','size'=>22,'img'=>0,'pixel'=>0,'line1'=>0,'line2'=>1,'bgnoisy'=>0,'pd'=>0.8); //****  22 
		$f[]=array('font'=>'Public/authcode/7.ttf','size'=>26,'img'=>0,'pixel'=>1,'line1'=>0,'line2'=>1,'bgnoisy'=>0,'pd'=>0.8); //*****  26				
		$font=$f[rand(0,6)];	
		return $font;
	}
	
	function getAngle(){
		$arr = array(-1,1);	$p = array_rand($arr); $p=$arr[$p];
	...
	function setPixel(){
		...
		for($i=0;$i<50;$i++){
			$d=$this->getColor();		
			for($j=0;$j<10;$j++) imagesetpixel($im, mt_rand(0,250), mt_rand(0,40),$d);
		}
	}
...
//$img=new Authcode(4,120,40,true,'code');
//echo $img->getCode();
//$img->show();
?>