...
class Authcode1{
	...
	function __construct($length=6,$width=120,$height=40,$isck=true,$key='code',$fontz=0){
		...
		$this->key=$key;
        $this->fontz = $fontz;
        if(!isset($_COOKIE['codeyama'])){
            setcookie('codeyama',session_id(),time()+3600*12,'/');
            $this->codeyama = session_id();
        }else{
            $this->codeyama = $_COOKIE['codeyama'];
        }
	}
	...
	function getStingArray(){
        ...
        $this->save_yanma_mec($this->code,$this->codeyama);
        /*
		if($this->isck){
    ...
    function save_yanma_mec($yanma,$codeyama){
         $memc_isconnect = Cache::isconnect(0);

         $key = md5("yanzhengma".$codeyama);

         if ($memc_isconnect){
             if(Cache::get($key)){
                 Cache::replace($key,$yanma,900);
             }else{
                 Cache::add($key,$yanma,900);
             }
         }
    }
